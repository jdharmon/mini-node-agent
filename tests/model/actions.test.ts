...
const FORMAT = "{{error}}";
const tools = createDefaultTools({ shell: "auto", env: {} });
...
describe("tool actions", () => {
  ...
  it("rejects tool=read without a path argument", () => {
    expect(() => parseTextToolActions("```tool=read\n```tool", FORMAT, tools)).toThrow(FormatError);
...
function formatErrorContent(content: string): string {
  try {
    parseTextToolActions(content, FORMAT, tools);
